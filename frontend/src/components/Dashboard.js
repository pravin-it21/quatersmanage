...
import QuartersBooking from "./QuartersBooking";
import ManageBooking from "./ManageBooking"; // Import ManageBooking component
import AdminManageBookings from "./AdminManageBookings"; // Import AdminManageBookings component

import './ManageBooking.css'; // Ensure the CSS is imported
...
function Dashboard(props) {
    ...
    return (
      <Router>
        <Switch>
		  ...
		  <Route path="/dashboard/quarters-booking">
  ...
  {role === "admin" ? <AdminManageBookings /> : <ManageBooking user={props.user} />}
          ...
          <Route path="/complaint/resolve/:id" component={(props) => (
            <div>
              <Header />
              <div style={{ marginTop: "100px" }}>
                <center><h1>Resolve Complaints!</h1></center>
              </div>
              <ResolveComp {...props} className="resolvecomplaint" />
              <br /><br />
            </div>
          )}>
          </Route>
          ...
          <Route path="/dashboard/housekeeping">
            <div className="complaintLedger">
              <Header />
              <Housekeeping role={role} id={id} />
              <br /><br />
            </div>
          </Route>

          <Route path="/housekeeping/request/:uid/:sid" component={(props) => (
            <div>
              <Header />
              <div style={{ margin: "100px" }}></div>
              <RequestHousekeeping {...props} id={id} role={role} className="resolvecomplaint" />
              <br />
            </div>
          )}>
          </Route>
